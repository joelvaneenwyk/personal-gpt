...
```bash
PGPT_PROFILES=local poetry run python -m personal_gpt
```
...
`PGPT_PROFILES=openai poetry run python -m personal_gpt`
...
`PGPT_PROFILES=sagemaker poetry run python -m personal_gpt`
...
`PGPT_PROFILES=ollama poetry run python -m personal_gpt`
